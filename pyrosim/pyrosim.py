...
def Prepare_Link_Dictionary(bodyID):
    ...
    for jointIndex in range( 0 , p.getNumJoints(bodyID)+1 ):
        ...
        print("LOOK HERE LINKNAME = " + linkName)
...
def Send_Link(name, pos, size, objectType, color, cname):
    ...
    if filetype == SDF_FILETYPE:
        ...
        link = LINK_URDF(name,pos,size,color,cname)
...
def Send_Cube(name="default",pos=[0,0,0],size=[1,1,1], color='    <color rgba="0 1.0 1.0 1.0"/>', cname = "Cyan"):

    Send_Link(name,pos,size,"box", color, cname)
