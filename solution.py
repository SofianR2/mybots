...
class SOLUTION:
  ...
  def Create_Body(self):
    ...
    for i in range(self.max):
      ...
      current = i
      nex = i + 1
      pyrosim.Send_Cube(name= str(i), pos=[x,y,z+height_offset-0.5] , size=[length, width, height])
      if(i!=self.max-1):
        pyrosim.Send_Joint(name = str(current) + "_" + str(nex), parent= str(current) , child = str(nex) , type = "revolute", position = [x+(width/2)+0.5,y,z+height_offset], jointAxis = "0 1 0")
        height_offset = -0.5
